...
import { X, ImagePlus, Trash2, VideoIcon } from 'lucide-react';
...
export const BlockContextMenu: React.FC<BlockContextMenuProps> = ({ blockId, x, y, onClose }) => {
  ...
  const handleVideoUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (ev) => {
      saveToHistory();
      updateBlock(blockId, { contentVideo: ev.target?.result as string, contentImage: '' });
    };
    reader.readAsDataURL(file);
    e.target.value = '';
  };
  ...
  return (
    ...
    >
      ...
      <div className="p-3 space-y-4 overflow-y-auto max-h-[80vh]">
        ...
        {/* ── Content Video ── */}
        <div className="space-y-2">
          <div className="text-xs font-semibold text-foreground">Content Video</div>
          <input
          ...
          {block.contentVideo ? (
            <div className="space-y-1.5">
              <div className="relative w-full rounded-lg overflow-hidden bg-muted">
                ...
                />
                <button
                  onClick={() => { saveToHistory(); updateBlock(blockId, { contentVideo: '' }); }}
                  className="absolute top-1 right-1 p-0.5 bg-destructive text-destructive-foreground rounded-full hover:opacity-80"
                >
                  <X className="w-3 h-3" />
                </button>
